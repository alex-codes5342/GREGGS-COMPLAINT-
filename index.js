...
async function logGenerationToWebhook(user) {
  ...
  try {
    await fetch(webhookUrl, {
      ...
      body: JSON.stringify({
        content: `<@${user.id}> generated a complaint.`,
        allowed_mentions: { users: [] }, // logs the name without actually pinging them
      }),
    });
  } catch (err) {
...
async function handleGenerateButton(interaction) {
  ...
  logGenerationToWebhook(interaction.user); // fire-and-forget, don't block the reply
